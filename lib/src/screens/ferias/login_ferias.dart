...
class LoginScreenFerias extends StatefulWidget {
  const LoginScreenFerias({super.key});

  @override
  State<LoginScreenFerias> createState() => _LoginScreenFeriasState();
}

class _LoginScreenFeriasState extends State<LoginScreenFerias> {
  ...
  Future login() async {
    logIn = await dio.post('${GlobalVariable.baseUrlApi}scav/v1/user/login',
        data: {"email": correo.text, "password": password.text, "tipo": "3"});
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    Center(
                        child: Text(
                      "SISTEMA DE CONCURSOS, FERIAS Y EXPOSICIONES",
                      style: TextStyle(
                          ...
                          child: TextFormField(
                            ...
                            onEditingComplete: () async {
                              if (_formKey.currentState!.validate()) {
                                ...
                                if (await login() == true) {
                                  cookie.set('idFer', logIn.data['id_usuario'],
                                      maxAge: const Duration(days: 6));
                                  switch (logIn.data['rol']) {
                                    case 'Administrador':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter.to("/homeFerias");
                                      break;
                                    case 'Capturista':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter
                                          .to("/homeCapacitacionFer");
                                      break;
                                    case 'Consultor':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter
