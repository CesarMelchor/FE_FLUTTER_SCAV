...
class LoginScreenCredenciales extends StatefulWidget {
  const LoginScreenCredenciales({super.key});

  @override
  State<LoginScreenCredenciales> createState() =>
      _LoginScreenCredencialesState();
}

class _LoginScreenCredencialesState extends State<LoginScreenCredenciales> {
  ...
  Future login() async {
    logIn = await dio.post('${GlobalVariable.baseUrlApi}scav/v1/user/login',
        data: {"email": correo.text, "password": password.text, "tipo": "1"});
              ...
              child: Row(
                ...
                children: [
                  ElevatedButton.icon(
                      ...
                      onPressed: () {
                        context.vRouter.to("/homeOffline");
                      },
                      label: Text(
                        "OFFLINE",
                                ...
                                if (await login() == true) {
                                  ...
                                  switch (logIn.data['rol']) {
                                    case 'Administrador':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter
                                          .to("/homeCredencializacion");
                                      break;
                                    case 'Capturista':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter
                                          .to("/homeCredencializacionCap");
                                      break;
                                    case 'Consultor':
                                      // ignore: use_build_context_synchronously
                                      context.vRouter
                                          .to("/homeCredencializacionCon");
                                      break;
                                    default:
                                  }
                                }
